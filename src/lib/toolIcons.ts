import toolTiktok from "@/assets/icons/tool-tiktok.png";
import toolAiChat from "@/assets/icons/tool-ai-chat.png";
import toolVoice from "@/assets/icons/tool-voice.png";
import toolImageCompress from "@/assets/icons/tool-image-compress.png";
import toolQr from "@/assets/icons/tool-qr.png";
import toolPassword from "@/assets/icons/tool-password.png";
import toolUnit from "@/assets/icons/tool-unit.png";
import toolCurrency from "@/assets/icons/tool-currency.png";
import toolTextCase from "@/assets/icons/tool-text-case.png";
import toolWordCount from "@/assets/icons/tool-word-count.png";
import toolColor from "@/assets/icons/tool-color.png";
import toolRandom from "@/assets/icons/tool-random.png";
import toolStopwatch from "@/assets/icons/tool-stopwatch.png";
import toolWeather from "@/assets/icons/tool-weather.png";
import toolHash from "@/assets/icons/tool-hash.png";
import toolBase64 from "@/assets/icons/tool-base64.png";
import toolBluetooth from "@/assets/icons/tool-bluetooth.png";
import toolWifi from "@/assets/icons/tool-wifi.png";
import toolIpLocation from "@/assets/icons/tool-ip-location.png";
import toolCodeEditor from "@/assets/icons/tool-code-editor.png";
import toolImagePdf from "@/assets/icons/tool-image-pdf.png";
import toolCalculator from "@/assets/icons/tool-calculator.png";
import toolBmi from "@/assets/icons/tool-bmi.png";
import toolAge from "@/assets/icons/tool-age.png";
import toolTyping from "@/assets/icons/tool-typing.png";
import toolSnake from "@/assets/icons/tool-snake.png";
import toolLuckyWheel from "@/assets/icons/tool-lucky-wheel.png";
import toolTodo from "@/assets/icons/tool-todo.png";
import toolNotes from "@/assets/icons/tool-notes.png";
import toolPomodoro from "@/assets/icons/tool-pomodoro.png";
import toolJson from "@/assets/icons/tool-json.png";
import toolGradient from "@/assets/icons/tool-gradient.png";
import toolCoin from "@/assets/icons/tool-coin.png";
import toolTip from "@/assets/icons/tool-tip.png";
import toolLoan from "@/assets/icons/tool-loan.png";
import toolTranslator from "@/assets/icons/tool-translator.png";
// New icons
import toolRegex from "@/assets/icons/tool-regex.png";
import toolMarkdown from "@/assets/icons/tool-markdown.png";
import toolEmoji from "@/assets/icons/tool-emoji.png";
import toolDraw from "@/assets/icons/tool-draw.png";
import toolBarcode from "@/assets/icons/tool-barcode.png";
import toolPercent from "@/assets/icons/tool-percent.png";
import toolDateDiff from "@/assets/icons/tool-date-diff.png";
import toolCountdown from "@/assets/icons/tool-countdown.png";
import toolPaletteGen from "@/assets/icons/tool-palette-gen.png";
import toolShadow from "@/assets/icons/tool-shadow.png";
import toolDiff from "@/assets/icons/tool-diff.png";
import toolUrl from "@/assets/icons/tool-url.png";
import toolBinary from "@/assets/icons/tool-binary.png";
import toolRoman from "@/assets/icons/tool-roman.png";
import toolTemperature from "@/assets/icons/tool-temperature.png";
import toolMorse from "@/assets/icons/tool-morse.png";
import toolDice from "@/assets/icons/tool-dice.png";
import toolRps from "@/assets/icons/tool-rps.png";
import toolMemory from "@/assets/icons/tool-memory.png";
import toolTicTacToe from "@/assets/icons/tool-tictactoe.png";
import toolQuote from "@/assets/icons/tool-quote.png";
import toolScreenRecorder from "@/assets/icons/tool-screen-recorder.png";
import toolHabit from "@/assets/icons/tool-habit.png";
import toolCharmap from "@/assets/icons/tool-charmap.png";
import toolOcr from "@/assets/icons/tool-ocr.png";
import toolBgRemove from "@/assets/icons/tool-bgremove.png";
import toolAiMath from "@/assets/icons/tool-aimath.png";
import toolEnhance from "@/assets/icons/tool-enhance.png";
import toolQuiz from "@/assets/icons/tool-quiz.png";
import toolLorem from "@/assets/icons/tool-lorem.png";
import toolMeme from "@/assets/icons/tool-meme.png";
import toolAudioRecorder from "@/assets/icons/tool-audio-recorder.png";
import toolFlashcard from "@/assets/icons/tool-flashcard.png";
import toolBudget from "@/assets/icons/tool-budget.png";
import toolPixelArt from "@/assets/icons/tool-pixel-art.png";
import toolChess from "@/assets/icons/tool-chess.png";
import toolSudoku from "@/assets/icons/tool-sudoku.png";
import tool2048 from "@/assets/icons/tool-2048.png";
import toolPdfMerge from "@/assets/icons/tool-pdf-merge.png";

import { ToolId } from "@/lib/tools";

export const toolIcons: Partial<Record<ToolId, string>> = {
  tiktok: toolTiktok,
  ai_chat: toolAiChat,
  voice: toolVoice,
  image_compress: toolImageCompress,
  qr: toolQr,
  password: toolPassword,
  unit: toolUnit,
  currency: toolCurrency,
  text_case: toolTextCase,
  word_count: toolWordCount,
  color: toolColor,
  random: toolRandom,
  stopwatch: toolStopwatch,
  weather: toolWeather,
  hash: toolHash,
  base64: toolBase64,
  bluetooth: toolBluetooth,
  wifi: toolWifi,
  ip_location: toolIpLocation,
  code_editor: toolCodeEditor,
  image_pdf: toolImagePdf,
  calculator: toolCalculator,
  bmi_calculator: toolBmi,
  age_calculator: toolAge,
  typing_test: toolTyping,
  snake_game: toolSnake,
  lucky_wheel: toolLuckyWheel,
  todo_list: toolTodo,
  notes: toolNotes,
  pomodoro: toolPomodoro,
  json_formatter: toolJson,
  gradient_generator: toolGradient,
  flip_coin: toolCoin,
  tip_calculator: toolTip,
  loan_calculator: toolLoan,
  translator: toolTranslator,
  regex_tester: toolRegex,
  markdown_preview: toolMarkdown,
  emoji_picker: toolEmoji,
  draw_board: toolDraw,
  barcode_gen: toolBarcode,
  percentage_calc: toolPercent,
  date_diff: toolDateDiff,
  countdown_timer: toolCountdown,
  color_palette: toolPaletteGen,
  css_shadow: toolShadow,
  text_diff: toolDiff,
  url_encoder: toolUrl,
  binary_converter: toolBinary,
  roman_numeral: toolRoman,
  temperature: toolTemperature,
  morse_code: toolMorse,
  dice_roller: toolDice,
  rock_paper: toolRps,
  memory_game: toolMemory,
  tic_tac_toe: toolTicTacToe,
  quote_gen: toolQuote,
  screen_recorder: toolScreenRecorder,
  habit_tracker: toolHabit,
  character_map: toolCharmap,
  image_to_text: toolOcr,
  bg_remover: toolBgRemove,
  ai_math: toolAiMath,
  image_enhancer: toolEnhance,
  quiz: toolQuiz,
  lorem_ipsum: toolLorem,
  meme_maker: toolMeme,
  audio_recorder: toolAudioRecorder,
  flashcard_maker: toolFlashcard,
  budget_planner: toolBudget,
  pixel_art: toolPixelArt,
  chess_game: toolChess,
  sudoku: toolSudoku,
  game_2048: tool2048,
  pdf_merger: toolPdfMerge,
};
